...
def read_img():
    gesture2label = {'up': 0, 'down': 1, 'left': 2, 'right': 3, 'larger': 4, 'smaller': 5}
    imgs = []
    labels = []
    for dir_name in gesture2label.keys():
        abs_path = path + dir_name + '/'
        for img_name in os.listdir(abs_path):
            img = cv2.imread(abs_path + img_name)
            img = cv2.resize(img, (w, h))
            imgs.append(img)
            labels.append(gesture2label[dir_name])

    return np.asarray(imgs, np.float32), np.asarray(labels, np.int32)


data, label = read_img()  # data 4038*(100,100,3)  label 4038个0~5
